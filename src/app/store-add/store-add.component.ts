 ...
 loadStoreDetails(storeName: string) {
  this.storeService.getStoreWithProducts(storeName).subscribe(storeData => {
    this.model.storeName = storeData.StoreName;  
    this.model.storeTypeId = storeData.StoreTypeId; 
    this.model.createdBy = storeData.CreatedBy; 

    
    if (storeData.Products && Array.isArray(storeData.Products)) {
      this.model.products = storeData.Products.map((prod: any) => ({
